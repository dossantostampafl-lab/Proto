...
class DecisionMemoryEntry(BaseModel):
    """Fact-only decision lineage with explicit missing evidence."""
    ...
    async def record_outcome(self, outcome: DecisionOutcome) -> DecisionOutcome:
        ...
        async with self.session_factory() as session:
            ...
            result = await session.execute(
                ...
                .values(resolved_at=outcome.resolved_at, outcome_json=serialized)
                .execution_options(synchronize_session=False)
            )
            if result.rowcount != 1:
                await session.rollback()
                current = await session.get(DecisionMemoryRecord, key)
                if current is None or current.outcome_json is None:
                    raise RuntimeError("decision outcome update lost its target")
                stored = DecisionOutcome.model_validate_json(current.outcome_json)
                if stored != outcome:
                    raise ValueError("decision outcome is immutable once recorded")
                return stored
            await session.commit()
