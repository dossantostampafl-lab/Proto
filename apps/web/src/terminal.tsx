...
function CandleChart({ candles }: { candles: Candle[] }) {
  ...
  const yTicks = Array.from({length:5}, (_,i) => hi - i * span / 4);
  const timeIndexes = Array.from(new Set([0, Math.floor((candles.length-1)/3), Math.floor((candles.length-1)*2/3), candles.length-1]));
  const formatTime = (t:number) => new Date(t).toISOString().slice(11,19);
  const priceDigits = Math.abs(last.close) >= 1000 ? 0 : Math.abs(last.close) >= 100 ? 2 : 4;
  return <div className="marketChartWrap"><svg className="candleChart" viewBox="0 0 760 238" preserveAspectRatio="none" role="img" aria-label="Live 5-second OHLC chart with price and UTC time axes">
    {yTicks.map((tick,i) => <g key={`y-${i}`}><line x1={plot.left} x2={plot.right} y1={y(tick)} y2={y(tick)} className="gridLine"/><text x="748" y={y(tick)+3} textAnchor="end" className="axisLabel">{num(tick,priceDigits)}</text></g>)}
    {timeIndexes.map(i => <g key={`x-${i}`}><line x1={x(i)} x2={x(i)} y1={plot.top} y2={plot.bottom} className="verticalGrid"/><text x={x(i)} y="230" textAnchor={i===0?"start":i===candles.length-1?"end":"middle"} className="axisLabel">{formatTime(candles[i].t)}</text></g>)}
    {candles.map((c, i) => { const cx=x(i); const rise=c.close>=c.open; return <g key={c.t} className={rise?"cUp":"cDown"}><line x1={cx} x2={cx} y1={y(c.high)} y2={y(c.low)}/><rect x={cx-3.3} y={Math.min(y(c.open),y(c.close))} width="6.6" height={Math.max(1.8,Math.abs(y(c.open)-y(c.close)))} rx=".6"/></g>; })}
    <line x1={plot.left} x2={plot.right} y1={y(last.close)} y2={y(last.close)} className="lastPriceLine"/>
    <rect x="684" y={y(last.close)-8} width="68" height="16" rx="2" className="lastPriceBox"/>
    <text x="748" y={y(last.close)+3} textAnchor="end" className="lastPriceText">{num(last.close,priceDigits)}</text>
  </svg><div className="chartMeta"><span>5S UTC BUCKETS</span><span>O {usd(last.open)}</span><span>H {usd(last.high)}</span><span>L {usd(last.low)}</span><span>C {usd(last.close)}</span><span>{candles.length} BARS</span></div></div>;
}
...
function ResearchField({ rows, edge }: { rows: LifecycleRow[]; edge: number }) {
  ...
  return <div className="researchField"><svg viewBox="0 0 760 300" role="img" aria-label="Synthetic expiry risk field">
    <defs><linearGradient id="riskFieldGradient" x1="0" x2="1"><stop offset="0" stopColor="#8b72ff"/><stop offset=".5" stopColor="#5ad8e6"/><stop offset="1" stopColor="#e5bd61"/></linearGradient></defs>
    ...
    {rows.slice(0,10).map((m,index)=>{const angle=clamp(m.model_probability)*Math.PI*1.7-Math.PI*.85;const radius=170+clamp(m.expiry_horizon_minutes/240)*80;const px=370+Math.cos(angle)*radius;const py=150+Math.sin(angle)*radius*.45;return <g key={m.market_id}><line x1={px} y1={py} x2="370" y2="150" stroke={m.net_edge>=0?"rgba(88,221,160,.34)":"rgba(239,100,109,.34)"}/><circle cx={px} cy={py} r={index===0?5:3.5} fill={m.net_edge>=0?"#58dda0":"#ef646d"}/></g>;})}
...
function EdgeHistory({ values, current }: { values: number[]; current: number | null }) {
  ...
  const lo=Math.min(...values,-0.01),hi=Math.max(...values,0.01),span=Math.max(hi-lo,1e-9),y=(v:number)=>88-(v-lo)/span*68;
  const points=values.map((v,i)=>`${10+i*180/Math.max(values.length-1,1)},${y(v)}`).join(" ");
  return <div className="edgeViz"><svg viewBox="0 0 200 100" role="img" aria-label="Synthetic net edge history"><line x1="10" x2="190" y1={y(0)} y2={y(0)} className="zeroLine"/><polyline points={points} fill="none" className="edgeLine"/><circle cx="190" cy={y(values[values.length-1])} r="3" className={values[values.length-1]>=0?"edgeDot goodDot":"edgeDot badDot"}/></svg><div className="vizFooter"><span>range {pct(lo,2)} → {pct(hi,2)}</span><b className={(current??0)>=0?"good":"bad"}>{current==null?"—":pct(current,2)}</b></div></div>;
}
...
function HawkesCascade({ series, hawkes }: { series: number[]; hawkes: Hawkes | null }) {
  ...
  const nodes=[{x:26,y:58,w:.55},{x:58,y:28,w:.72},{x:96,y:52,w:1},{x:134,y:28,w:.65},{x:170,y:58,w:.48},{x:58,y:82,w:.44},{x:134,y:82,w:.52}];
  const intensity=clamp(hawkes.current_intensity/Math.max(hawkes.current_intensity,hawkes.baseline_intensity,1e-9));
  return <div className="hawkesViz"><svg viewBox="0 0 196 108" role="img" aria-label="Synthetic Hawkes excitation cascade">{nodes.slice(1).map((n,i)=><line key={`l-${i}`} x1="96" y1="52" x2={n.x} y2={n.y} className="cascadeLink" opacity={.25+n.w*.45}/>)}{nodes.map((n,i)=><g key={`n-${i}`}><circle cx={n.x} cy={n.y} r={5+n.w*5} className={i===2?"cascadeNode core":"cascadeNode"}/><circle cx={n.x} cy={n.y} r={2.5} className="cascadePoint"/></g>)}</svg><div className="hawkesMeters"><span><i style={{width:`${intensity*100}%`}}/>intensity {num(hawkes.current_intensity,4)}</span><span><i style={{width:`${clamp(hawkes.branching_ratio)*100}%`}}/>branch {num(hawkes.branching_ratio,4)}</span><span><i style={{width:`${clamp(hawkes.event_probability)*100}%`}}/>event P {pct(hawkes.event_probability,1)}</span></div>{series.length>1&&<Spark values={series.slice(-36)} tone="research"/>}</div>;
}

function GreeksPanel({ greeks }: { greeks: Greeks | null }) {
  const items=[["Δ","DELTA",greeks?.market_probability_delta],["ν","VEGA",greeks?.volatility_vega],["κ","KAPPA",greeks?.imbalance_kappa],["θ","THETA",greeks?.time_theta]] as const;
