...
function ResearchField({ rows, edge }: { rows: LifecycleRow[]; edge: number }) {
  ...
  return <div className="researchField"><svg viewBox="0 0 760 300" role="img" aria-label="Synthetic expiry risk field">
    {[0,1,2,3,4,5].map(i => <ellipse key={i} cx="380" cy="150" rx={280-i*34} ry={110-i*11} fill="none" stroke="rgba(113,167,255,.14)" />)}
    <ellipse cx="520" cy="154" rx={90+clamp(Math.abs(edge)*120)} ry="88" fill="rgba(142,108,255,.05)" stroke="rgba(232,193,90,.65)" />
    {rows.slice(0,10).map(m => { const a = clamp(m.model_probability)*Math.PI*1.7-Math.PI*.85; const r=170+clamp(m.expiry_horizon_minutes/240)*80; const x=380+Math.cos(a)*r; const y=150+Math.sin(a)*r*.45; return <g key={m.market_id}><line x1={x} y1={y} x2="520" y2="154" stroke={m.net_edge>=0?"rgba(92,227,155,.42)":"rgba(240,95,101,.38)"}/><circle cx={x} cy={y} r={4} fill={m.net_edge>=0?"#5ce39b":"#f05f65"}/></g>; })}
...
function App() {
  ...
  useEffect(()=>{
    ...
    const refresh=async()=>{
      ...
      if(cancelled)return;
      if(h.ok&&h.data)setHealth(h.data); if(s.ok&&s.data){setStatus(s.data);setStatusAt(Date.now());} if(m.ok&&m.data)m.data.markets.forEach(ingest); if(p.ok&&p.data)setPortfolio(p.data); if(pa.ok&&pa.data)setPnl(pa.data);
      if(l.ok&&l.data){setLifecycle(l.data);setResearchState("available");setEdgeSeries(old=>{const next={...old};SYMBOLS.forEach(sym=>{const row=l.data!.markets.find(x=>x.symbol===sym);if(row&&Number.isFinite(row.net_edge))next[sym]=append(next[sym],row.net_edge)});return next;});} else if(l.status===503){setLifecycle(null);setResearchState("disabled");} else if(l.status!==0)setResearchState("error");
      setAnalytics(old=>{const next={...old};aa.forEach((r,i)=>{if(r.ok&&r.data)next[SYMBOLS[i]]=r.data});return next;});
    };
    const connect=()=>{ if(cancelled)return; const ws=new WebSocket(`${WS_BASE}/ws/market-data`); socket.current=ws; ws.onopen=()=>!cancelled&&setWsConnected(true); ws.onmessage=e=>{try{const msg=JSON.parse(e.data as string) as Envelope;if(msg.type==="market-data"&&msg.data)ingest(msg.data as LiveFrame)}catch{}}; ws.onerror=()=>ws.close(); ws.onclose=()=>{if(!cancelled){setWsConnected(false);reconnect=window.setTimeout(connect,1500)}}; };
  ...
  return <main className="terminal" data-section="COMMAND" data-feed={liveFresh?"live":"stale"}>
    ...
    <section className="grid"><aside><article className="panel"><header>L1 ORDER BOOK <Badge kind="live"/></header>{active?<div className="book"><div className="ask"><span>ASK</span><b>{usd(active.ask)}</b><i>{num(active.ask_size,5)}</i></div><strong className="mid">{usd(active.mid)}</strong><div className="bid"><span>BID</span><b>{usd(active.bid)}</b><i>{num(active.bid_size,5)}</i></div></div>:<div className="empty">Waiting for live quote…</div>}</article><article className="panel"><header>ORDER FLOW <Badge kind="live"/></header><div className="flow"><div><i style={{left:`${clamp(((a?.current_imbalance??0)+1)/2)*100}%`}}/></div><strong>{a?num(a.current_imbalance,3):"—"}</strong><span>micro {a?usd(a.current_microprice):"—"}</span><span>vol {a?pct(a.realized_volatility):"—"}</span><span>samples {a?.sample_count??"—"}</span></div></article></aside>
      ...
      <aside><article className="panel"><header>EDGE HISTORY <Badge kind="research"/></header>{researchAvailable?<><Spark values={edgeSeries[selected]} tone="research"/><div className="rows"><span>decision<b>{selectedMarket?.edge_decision??"—"}</b></span><span>net edge<b className={edge>=0?"good":"bad"}>{selectedMarket?pct(edge,2):"—"}</b></span></div></>:<div className="empty">Research gate closed.</div>}</article><article className="panel"><header>HAWKES CASCADE <Badge kind="research"/></header>{researchAvailable?<><Spark values={hawkesSeries[selected]} tone="research"/><div className="rows"><span>intensity<b>{hawkes?num(hawkes.current_intensity,4):"—"}</b></span><span>branching<b>{hawkes?num(hawkes.branching_ratio,4):"—"}</b></span><span>event P<b>{hawkes?pct(hawkes.event_probability,1):"—"}</b></span></div></>:<div className="empty">Research gate closed.</div>}</article><article className="panel"><header>SYNTHETIC GREEKS <Badge kind="research"/></header><div className="greeks"><span>Δ<b>{greeks?num(greeks.market_probability_delta,4):"—"}</b></span><span>ν<b>{greeks?num(greeks.volatility_vega,4):"—"}</b></span><span>κ<b>{greeks?num(greeks.imbalance_kappa,4):"—"}</b></span><span>θ<b>{greeks?num(greeks.time_theta,4):"—"}</b></span></div></article></aside></section>
