...
function Candles({ values }: { values: number[] }) {
  ...
  return (
    <svg className="candleChart" viewBox="0 0 620 205" preserveAspectRatio="none">
      ...
      {groups.map((g, i) => {
        ...
        return (
          <g key={i} className={up ? "cUp" : "cDown"}>
            <line x1={x} x2={x} y1={y(high)} y2={y(low)} />
            <rect x={x - 4} y={bodyY} width="8" height={bodyH} />
          </g>
        );
      })}
    </svg>
  ...
  return (
    <div className="torusWrap">
      <svg className="torus" viewBox="0 0 900 470" preserveAspectRatio="xMidYMid meet">
        ...
        {Array.from({ length: 18 }).map((_, i) => { const angle = (i / 18) * Math.PI * 2; const x = 465 + Math.cos(angle) * 285; const y = 236 + Math.sin(angle) * 142; return <line key={i} x1="465" y1="236" x2={x} y2={y} stroke="rgba(86,166,217,.09)" />; })}
        ...
        {lifecycle.slice(0, 8).map((m, i) => { const x = 240 + i * 68; const y = 160 + Math.sin(i * 0.9) * 62; return <g key={m.market_id}><line x1={x} y1={y} x2="610" y2="245" stroke={m.net_edge >= 0 ? "rgba(92,227,155,.45)" : "rgba(240,95,101,.4)"} /><circle cx={x} cy={y} r={3 + clamp(Math.abs(m.net_edge) * 40, 0, 5)} fill={m.net_edge >= 0 ? "#5ce39b" : "#f05f65"} /></g>; })}
      </svg>
      <div className="torusCore"><small>EDGE FIELD</small><strong>{pct(edge, 2)}</strong><span>{lifecycle.length} markets mapped</span></div>
    </div>
  );
}
...
function GreeksField({ g, values }: { g: SyntheticGreeks | null; values: number[] }) {
  ...
  return <div className="fieldWrap"><svg viewBox="0 0 520 210" preserveAspectRatio="none" className="fieldSvg">
    ...
    {Array.from({ length: 18 }).map((_, i) => { const yy = 105 + (i - 9) * 5; const power = vals[i % 4]; const end = 480 - Math.abs(power) * 90; return <line key={i} x1="15" y1={yy} x2={end} y2={105 + (i - 9) * 1.2} stroke={power >= 0 ? "rgba(92,227,155,.55)" : "rgba(240,95,101,.48)"} strokeWidth={1 + Math.abs(power) * 4} />; })}
    {values.slice(-10).map((v, i) => { const base = values[Math.max(0, values.length - 10)] || v; const x = 150 + i * 32; const r = 4 + clamp(Math.abs(v / base - 1) * 220, 0, 10); return <circle key={i} cx={x} cy={108 + (i % 3) * 7} r={r} fill="none" stroke="rgba(232,193,90,.8)" strokeWidth="2" />; })}
  </svg></div>;
}
